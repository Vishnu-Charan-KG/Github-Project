...
import { FaMapMarkerAlt, FaBuilding, FaLink, FaCalendarAlt, FaGithub, FaUsers, FaUserPlus, FaCode, FaFileCode } from "react-icons/fa";
...
const ProfileViewer = () =>{
    ...
    const stats = [
        ...
        { icon: FaFileCode, label: 'Gists', value: profile?.public_gists, bgClass: 'from-orange-600/20 to-orange-800/20', borderClass: 'border-orange-500/30 hover:border-orange-400/50', iconClass: 'text-orange-400' }
    ];

    const infoItems = [
        { condition: profile?.company, icon: FaBuilding, label: 'Company', value: profile?.company, iconClass: 'text-blue-400' },
        { condition: profile?.location, icon: FaMapMarkerAlt, label: 'Location', value: profile?.location, iconClass: 'text-red-400' },
        { condition: profile?.blog, icon: FaLink, label: 'Website', value: profile?.blog, iconClass: 'text-green-400', isLink: true }
    ];

    return(
        <div className={`py-8 px-4 ${hasContent? 'min-h-full': 'flex h-full items-center justify-center'}`}>
                    ...
                    <div className="bg-linear-to-br from-gray-800 via-gray-800 to-gray-900 rounded-2xl shadow-2xl overflow-hidden mb-8 border border-gray-700/50 backdrop-blur-sm">
                        <div className="bg-linear-to-r from-blue-600/20 via-purple-600/20 to-pink-600/20 p-6 border-b border-gray-700/50">
                            <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
                                <div className="relative">
                                    ...
                                    <img src={profile.avatar_url} alt={profile.name || profile.login} className="relative w-32 h-32 md:w-40 md:h-40 rounded-full border-4 border-gray-700 shadow-xl" />
                                </div>
                                <div className="flex-1 text-center md:text-left">
                                    <h2 className="text-3xl md:text-4xl font-bold text-white mb-2">{profile.name || profile.login}</h2>
                                    <p className="text-gray-400 text-lg mb-3 flex items-center justify-center md:justify-start gap-2"><FaGithub className="text-gray-500"/>@{profile.login}</p>
                                    {profile.bio && <p className="text-gray-300 text-base mb-4 max-w-4xl">{profile.bio}</p> }
                                    <a href={profile.html_url} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-linear-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 text-white rounded-lg transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105"><FaGithub />View on Github</a>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="p-6 bg-gray-900/50">
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                                ...
                                {stats.map(({ icon:Icon, label, value, bgClass, borderClass, iconClass}) =>(
                                    <div key={label} className={`bg-linear-to-br ${bgClass} p-4 rounded-xl border ${borderClass} transition-all duration-200 hover:scale-105`}>
                                        <div className={`flex items-center gap-2 ${iconClass} mb-2`}><Icon className="text-lg" /><div className="text-gray-400 text-sm font-medium">{label}</div></div>
                                        <div className="text-3xl font-bold text-white">{value}</div>
                                    </div>
                                ))}
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4 border-t border-gray-700/50">{infoItems.map(({ condition, icon:Icon , label,value, iconClass, isLink}) => condition && (
                            ...
                            <div className="flex items-center gap-3 text-gray-300 p-3 rounded-lg bg-gray-800/50 hover:bg-gray-800 transition-colors">
                                ...
                                <div><div className="text-xs text-gray-500 uppercase tracking-wide">Joined</div><div className="font-semibold">{formatDate(profile.created_at)}</div></div>
