...
const RepoExplorer =() => {
    ...
    const handleFilterChange = (type, value) => {
        ...
        if(hasSearched && query.trim()) performSearch(1, type === 'sort'?value:sort, type==='order'? value:order)
    };

    const totalpages = Math.ceil(totalCount / 30);
    const containerClass= `py-8 px-4 ${hasSearched ? `min-h-full`: `h-full flex items-center justify-center`}`;
     ...
     return(
        <div className={containerClass}>
            <div className={`max-w-7xl mx-auto ${hasSearched ? '' : 'w-full'}`}>
                ...
                <SearchForm value={query} onChange={(e) => setQuery(e.target.value)} onSubmit={(e) => handleSearch(e,1)} placeholder="Search repositories (e.g., react, python, machine Learning)..." loading={loading} />
                ...
                {hasSearched && repos.length ===0 && !loading && !error && <div className="text-center py-12"><p className="text-gray-400 text-lg">No repositories found. Try a different query</p></div>}
            </div>
        </div>
     );
};
